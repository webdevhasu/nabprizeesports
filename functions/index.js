  ...
  try {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      transaction.set(playerRef, { 
        ...
        registeredAt: FieldValue.serverTimestamp(), 
        status: 'registered',
        isSquad: (tournament.matchType || '').toLowerCase().includes('squad') || (!tournament.matchType && !!teamName),
        isDuo: (tournament.matchType || '').toLowerCase().includes('duo'),
        isTeam: !!teamName,
        matchType: tournament.matchType || (teamName ? 'Team' : 'Solo'),
        teamName: teamName || null,
        teamLogo: teamLogo || null,
        teammates: teammates,
        teamSlot: (tournament.slotsFilled || 0) + 1
